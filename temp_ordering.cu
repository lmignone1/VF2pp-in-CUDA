    ...
    if(matrix[node * V + idx] == 1) {
        ...
        if(constMem[offset + idx] != -1) {
            ...
            for(int adjVertex2 = 0; adjVertex2 < V; adjVertex2++) {
                if(matrix[idx * V + adjVertex2] == 1 && constMem[offset + adjVertex2] != -1) {   // constMem contains mapping
...
void restoreState(Graph* g1, Graph* g2, State* state, int node, int candidate) {
    ...
    CUDA_CHECK_ERROR(cudaMemcpyAsync(d_matrix1, g1->matrix, g1->numVertices * g1->numVertices * sizeof(int), cudaMemcpyHostToDevice, stream1));
    CUDA_CHECK_ERROR(cudaMemcpyToSymbolAsync(constMem, state->mapping1, g1->numVertices * sizeof(int), 0, cudaMemcpyHostToDevice, stream1));
    CUDA_CHECK_ERROR(cudaMemcpyAsync(d_T1, state->T1, g1->numVertices * sizeof(int), cudaMemcpyHostToDevice, stream1));
    CUDA_CHECK_ERROR(cudaMemcpyAsync(d_T1_out, state->T1_out, g1->numVertices * sizeof(int), cudaMemcpyHostToDevice, stream1));

    int gridSize = (g1->numVertices + blockSize - 1) / blockSize;

    restoreStateKernel<<<gridSize, blockSize, 0, stream1>>>(d_matrix1, g1->numVertices, node, d_T1, d_T1_out, 0);
    ...
    CUDA_CHECK_ERROR(cudaMemcpyAsync(d_matrix2, g2->matrix, g2->numVertices * g2->numVertices * sizeof(int), cudaMemcpyHostToDevice, stream2));
    CUDA_CHECK_ERROR(cudaMemcpyToSymbolAsync(constMem, state->mapping2, g2->numVertices * sizeof(int), g1->numVertices * sizeof(int), cudaMemcpyHostToDevice, stream2));
    CUDA_CHECK_ERROR(cudaMemcpyAsync(d_T2, state->T2, g2->numVertices * sizeof(int), cudaMemcpyHostToDevice, stream2));
    CUDA_CHECK_ERROR(cudaMemcpyAsync(d_T2_out, state->T2_out, g2->numVertices * sizeof(int), cudaMemcpyHostToDevice, stream2));

    gridSize = (g2->numVertices + blockSize - 1) / blockSize;

    restoreStateKernel<<<gridSize, blockSize, 0, stream2>>>(d_matrix2, g2->numVertices, candidate, d_T2, d_T2_out, g1->numVertices);
...
__global__ void findCandidatesKernel(int coveredNeighborsSize, int g1_label, int maxSizeCandidates,
                                    ...
                                    int g2_numVertices, int* commonNodes, int* g2_matrix, int* g2_nodesToLabel, int offset) {
...
int* findCandidates(Graph* g1, Graph* g2, State* state, int node, int* sizeCandidates) {
    ...
    int* coveredNeighbors = (int*)malloc(g1->degrees[node] * sizeof(int));
    ...
    CUDA_CHECK_ERROR(cudaMemcpy(&coveredNeighborsSize, d_coveredNeighborsSize, sizeof(int), cudaMemcpyDeviceToHost));
    CUDA_CHECK_ERROR(cudaMemcpy(coveredNeighbors, d_coveredNeighbors, g1->degrees[node] * sizeof(int), cudaMemcpyDeviceToHost));

    CUDA_CHECK_ERROR(cudaMemcpyToSymbol(constMem, coveredNeighbors, g1->degrees[node] * sizeof(int), 0));
    CUDA_CHECK_ERROR(cudaMemcpyToSymbol(constMem, state->mapping1, g1->numVertices * sizeof(int), g1->degrees[node] * sizeof(int)));

    gridSize = (g2->numVertices + blockSize - 1) / blockSize;
    findCandidatesKernel<<<gridSize, blockSize, 0, stream2>>>(coveredNeighborsSize, g1_label, maxSizeCandidates, d_g2_vertexList, g1_degree,
        d_g2_degrees, d_T2_out, d_mapping2, d_candidates, d_candidateSize, g2->numVertices, d_commonNodes,
        d_g2_matrix, d_g2_nodesToLabel, g1->degrees[node]);
...
bool cutISO(Graph* g1, Graph* g2, State* state, int node1, int node2){
    ...
    int* neighbors2 = (int*)malloc(g2->degrees[node2] * sizeof(int));
    ...
    CUDA_CHECK_ERROR(cudaMemcpy(neighbors2, d_neighbors2, g2->degrees[node2] * sizeof(int), cudaMemcpyDeviceToHost));

    CUDA_CHECK_ERROR(cudaMemcpyToSymbol(constMem, neighbors2, g2->degrees[node2] * sizeof(int), 0));
    CUDA_CHECK_ERROR(cudaMemcpyToSymbol(constMem, g2->nodesToLabel, g2->numVertices * sizeof(int), g2->degrees[node2] * sizeof(int)));
    ...
    checkLabelsKernel<<<gridSize, blockSize, 0, stream3>>>(d_neighbors1, nbrSize1, nbrSize2,
        d_labelsNbr, g1->numVertices, d_g1_nodesToLabel, d_result, g2->degrees[node2]);
