...
void vf2pp(Graph*, Graph*, State*, int**);
...
bool testIsoMapping(State*, int, int);
...
int main(int argc, char* argv[]) {
    ...
    testOrdering(order, g1->numVertices);
    testIsoMapping(s, g1->numVertices, g2->numVertices);
...
bool testOrdering(int* order, int size) {
    int expected1[] = {8, 9, 2, 10, 11, 0, 16, 5, 15, 1, 6, 19, 4, 3, 7, 14, 17, 12, 18, 13};
    int expected2[] = {8, 9, 2, 11, 10, 0, 16, 5, 15, 1, 6, 19, 4, 3, 7, 14, 17, 12, 18, 13};
...
bool testIsoMapping(State* s, int size1, int size2) {
    ...
    for(int i = 0; i < size1; i++) {
        assert(s->mapping1[i] == expected[i]);
        assert(s->mapping2[i] == expected[i]);
    }
...
void vf2pp(Graph* g1, Graph* g2, State* state, int** order_test) {
