...
bool cutISO(Graph* g1, Graph* g2, State* state, int node1, int node2){
    ...
    bool result = checkLabels(g1, g2, neighbors1, neighbors2, nbrSize1, nbrSize2, labelsNbr);
    ...
    if(!result) {   // verifica almeno una corrispondenza G1 a G2
        free(neighbors1);
        free(neighbors2);
        free(labelsNbr);
        return true;
